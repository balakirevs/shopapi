...
describe 'Api::V1::UsersRequest', type: :request do
  let!(:user) { create(:user) }
  let(:uri_1) { 'http://api.shopapi.dev/v1/users/' }
  let(:uri_2) { "http://api.shopapi.dev/v1/users/#{user.id}" }
  ...
  describe 'POST #create' do
    ...
    context 'when is successfully created' do
      ...
      it 'creates user and saves it to db' do
        expect{ create_user_request uri_1 }.to change{ User.count }.by(1)
      end
    end
    ...
    context 'when is not created' do
      ...
      it "returns a client error 422('Unprocessable Entity') response" do
        create_user_request(uri_1, @invalid_user_attributes)
      ...
      it "doesn't create & save user when the 'email' field is empty" do
        ...
        expect{ create_user_request(uri_1, @invalid_user_attributes) }.to_not change{ User.count }
      end
    end
  end
  ...
  describe 'PUT/PATCH #update' do
    context 'when is successfully updated' do
      before(:each) do
        patch uri_2, params: { user: { email: 'newmail@example.com' } }, headers: { 'Authorization' => user.auth_token }
      end
    ...
    context 'when is not updated' do
      before(:each) do
        patch uri_2, params: { user: { email: 'bademail.com' } }, headers: { 'Authorization' => user.auth_token }
      end
